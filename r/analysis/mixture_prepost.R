...
model = bf(
  ...
  mu1 ~ 0 + Intercept + time * stimulus, # psychometric function
  mu2 ~ 0 + Intercept + time,            # guessing probability
  theta1 ~ 0 + Intercept + time,         # mixing proportion (lapse rate)
  family = mix
)
...
p_prior_ce = plot_ce(prior_fit, NA, 3)
...
posterior_fit = brm(model,
                    ...
                    backend = 'cmdstanr',
                    file = "mixture_prepost")

p_posterior_ce = plot_ce(posterior_fit, d_sim_summary, 3)

# parameter estimates
# -----------------------------------------------
# get_variables(posterior_fit)
pars = get_pars_wichmann_prepost(posterior_fit)
...
# -----------------------------------------------
# get_variables(posterior_fit)
pl_pars = get_pars_wichmann_prepost(posterior_fit)
